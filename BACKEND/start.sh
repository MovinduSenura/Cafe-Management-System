...
ENV_FILE="$SCRIPT_DIR/.env.compose"
...
function check_existing_containers() {
    # Check if cafe containers exist
    if docker ps -a --format '{{.Names}}' | grep -q "^cafe-mgmt-"; then
        print_info "Existing cafe management containers detected."
        echo ""
        echo "Choose an option:"
        echo "  1) Restart existing containers (preserves data)"
        echo "  2) Stop and remove existing containers, then start fresh"
        ...
        case $choice in
            1)
                print_info "Restarting existing containers..."
...
function stop_services() {
    ...
    if docker ps -a | grep -q 'cafe-mgmt-'; then
        print_info "Some cafe management containers still exist. Force removing them..."
        docker ps -a | grep 'cafe-mgmt-' | awk '{print $1}' | xargs -r docker rm -f
    fi
...
function check_status() {
    ...
    for service in "${services[@]}"; do
        health=$(docker inspect --format='{{.State.Health.Status}}' "cafe-mgmt-${service}" 2>/dev/null || echo "unknown")
...
function force_restart_services() {
    print_header
    print_info "Force restarting all microservices..."
    
    print_info "Stopping existing services..."
    docker-compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" down
    
    # Force remove any remaining cafe containers
    print_info "Ensuring all cafe management containers are removed..."
    docker ps -a | grep 'cafe-mgmt-' | awk '{print $1}' | xargs -r docker rm -f
    ...
    if [ $? -eq 0 ]; then
        print_success "All services force restarted successfully!"
    else
        print_error "Failed to force restart services"
        exit 1
    fi
}
...
function show_help() {
    ...
    echo "  force-restart Force stop and start all microservices"
